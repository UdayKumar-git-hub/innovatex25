...
const Register: React.FC = () => {
  ...
  const handlePayment = async () => {
    ...
    if (!isSdkReady || !window.cashfree) {
      setValidationError("Payment gateway is not ready. Please wait a moment or refresh the page.");
      setIsLoading(false);
      return;
    }
      ...
      <div className="max-w-4xl mx-auto px-6">
            ...
            {currentStep === 5 && (() => {
              ...
              return (
                <div>
                  ...
                  <div className="space-y-6">
                    {postPaymentError && (<div className="p-4 bg-red-100 border-l-4 border-red-500 text-red-700"><div className='flex'><AlertTriangle className='h-5 w-5 text-red-500 mr-3'/><div><p className="font-bold">Payment Error</p><p>{postPaymentError}</p></div></div></div>)}
                    ...
                    <div className="bg-white p-6 rounded-lg border border-gray-200">
                      ...
                      <div className="space-y-3 text-gray-700">
                        ...
                        <hr className="my-2"/><div className="flex justify-between font-semibold"><span>Subtotal</span><span>₹{paymentDetails.priceAfterDiscount.toLocaleString()}</span></div>
                        <div className="flex justify-between"><span>Platform Fee (5%)</span><span>+ ₹{paymentDetails.platformFee.toFixed(2)}</span></div>
                        <hr className="my-2 border-t-2 border-gray-300"/><div className="flex justify-between text-2xl font-bold text-gray-800 mt-2"><span>Total Amount Due</span><span>₹{paymentDetails.total.toFixed(2)}</span></div>
                      </div>
                    </div>
        ...
        <div className="flex justify-between mt-8">
          ...
          {currentStep < 5 ? (
            ...
            <button onClick={handlePayment} disabled={!formData.agreedToRules || isLoading || !isSdkReady} className="flex items-center justify-center px-8 py-3 bg-green-500 text-white rounded-lg font-semibold disabled:opacity-50 disabled:cursor-not-allowed hover:bg-green-600 transition-colors w-60">
              {isLoading ? (
