...
const Register: React.FC = () => {
  ...
  const handlePayment = async () => {
    ...
    const options = {
        ...
        description: `Fee for Team '${formData.teamName}' (${formData.teamSize} members)`,
        image: "https://i.ibb.co/bF2cK3s/innovate-x-logo.png",
        ...
        handler: async function (response: any) {
            ...
            try {
                ...
                // The email will be sent automatically by the Supabase trigger.
                // Now, just navigate to the success page.
                navigate('/success', { 
                    state: { 
        ...
        notes: {
            ...
            team_interests: JSON.stringify(formData.interests), // Richer data for your dashboard
        },
        theme: {
            color: "#FBBF24",
            backdrop_color: "rgba(0, 0, 0, 0.75)" // Darker backdrop for better contrast
        },
        modal: {
            ondismiss: function () {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white via-yellow-50 to-gray-100 py-16 font-sans">
      <div className="max-w-4xl mx-auto px-6">
        ...
        >
          <div className="bg-white/60 backdrop-blur-md p-6 rounded-2xl shadow-lg border border-yellow-200/50 max-w-3xl mx-auto">
            <p className="text-gray-700 text-lg leading-relaxed">
              Hey Hyderabad Innovators! Get ready for an epic experience where your ideas can shine. 
        ...
        <div className="mb-8">
              ...
              <React.Fragment key={step}>
                <div className="flex items-center">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold transition-all duration-300 ${
                    step <= currentStep ? 'bg-yellow-500 text-white shadow-md' : 'bg-gray-200 text-gray-500'
                  }`}>
          ...
          <div className="text-center text-sm text-gray-600 font-semibold tracking-wider">
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="bg-white/70 backdrop-blur-md p-8 rounded-2xl shadow-lg border border-yellow-200/50"
          >
            {currentStep === 1 && (
              <div>
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <input type="text" value={formData.teamName} onChange={(e) => handleInputChange('teamName', e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-transparent transition" placeholder="Enter your team name"/>
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-3">Your Team Size:</label>
                    <div className="grid grid-cols-3 gap-4">
                      {[2, 3, 4].map((size) => (
                        <label key={size} className={`flex items-center justify-center p-4 border rounded-lg cursor-pointer hover:bg-yellow-100/50 transition-colors ${formData.teamSize === size ? 'border-yellow-500 bg-yellow-100 ring-2 ring-yellow-400' : 'border-gray-300'}`}>
                          ...
                          <span className="font-medium text-gray-800">Team of {size}</span>
                        </label>
                      ))}
                    </div>
                  </div>
            ...
            {currentStep === 2 && (
              <div>
                ...
                <div className="space-y-8">
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-4">All teams will compete in all challenges. Get ready for an epic showdown!</label>
                  ...
                  <div>
                    ...
                    <textarea value={formData.superpower} onChange={(e) => handleInputChange('superpower', e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-transparent transition" rows={3} placeholder="e.g., Super creative, amazing planners, master strategists..."/>
                  </div>
                </div>
              </div>
            )}
                    ...
                    <div key={index} className="p-6 bg-gray-50 rounded-lg border border-gray-200">
                      ...
                      <div className="grid md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <input type="text" value={formData.members[index].fullName} onChange={(e) => handleMemberChange(index, 'fullName', e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-transparent transition" placeholder="Enter full name"/>
                        </div>
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">Grade:</label>
                          <select value={formData.members[index].grade} onChange={(e) => handleMemberChange(index, 'grade', e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-transparent transition disabled:bg-gray-200/70 disabled:cursor-not-allowed" disabled={index > 0}>
                        ...
                        <div>
                          ...
                          <input type="email" value={formData.members[index].email} onChange={(e) => handleMemberChange(index, 'email', e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-transparent transition" placeholder="Enter email address"/>
                        </div>
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-1">Phone Number:</label>
                          <input type="tel" value={formData.members[index].phoneNumber} onChange={(e) => handleMemberChange(index, 'phoneNumber', e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-transparent transition" placeholder="Enter phone number"/>
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <div>
                  ...
                  <div className="space-y-6">
                    <div className="bg-gradient-to-r from-yellow-100 to-yellow-200 p-6 rounded-lg border border-yellow-300">
                      ...
                      <p className="text-yellow-700 mb-2">Register before <strong>October 20th, 2025</strong> and get a <strong>₹50 discount per team!</strong></p>
                    ...
                    <div className="flex items-start p-4 bg-blue-50/60 rounded-lg border border-blue-200">
        ...
        <div className="flex justify-between mt-8">
          <button onClick={prevStep} disabled={currentStep === 1} className="flex items-center px-6 py-3 bg-gray-200 text-gray-700 rounded-lg font-semibold disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-300 transition-all transform hover:scale-105">
            <ChevronLeft className="w-5 h-5 mr-2" />Previous
          </button>
          {currentStep < 4 ? (
            <button onClick={nextStep} disabled={!isStepValid()} className="flex items-center px-6 py-3 bg-yellow-500 text-white rounded-lg font-semibold hover:bg-yellow-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-105 shadow-md">
              Next <ChevronRight className="w-5 h-5 ml-2" />
            </button>
          ) : (
            <button onClick={handlePayment} disabled={!formData.agreedToRules || isLoading} className="flex items-center justify-center px-8 py-3 bg-green-500 text-white rounded-lg font-semibold disabled:opacity-50 disabled:cursor-not-allowed hover:bg-green-600 transition-all w-60 transform hover:scale-105 shadow-lg">
        ...
        {/* --- Contact Info --- */}
        <div className="mt-16 text-center bg-white/60 backdrop-blur-md p-6 rounded-2xl shadow-lg border border-yellow-200/50">
          <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center justify-center"><MapPin className="w-5 h-5 text-yellow-500 mr-2" />Event Details</h3>
          <p className="text-gray-600">reelhaus.hyd | Hyderabad, Telangana</p>
          <div className="border-t my-4"></div>
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Questions? Contact the crew:</h3>
