...
if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
    console.error("Supabase URL or Anon Key is missing. Make sure they are set in your environment variables.");
} else {
...
const Register = () => {
    ...
    const [qrCodeSrc] = React.useState('https://i.postimg.cc/yx8CBxLJ/Whats-App-Image-2025-09-19-at-07-46-31-2821abad.jpg');
    const [upiDetails] = React.useState('udayl4905-2@okhdfcbank');
    ...
    const uploadScreenshot = async (file: File | null) => {
        ...
        const { error } = await supabase.storage.from("payments").upload(filePath, file);
    ...
    const handleFinalSubmission = async () => {
        ...
        if (!supabase) {
            setValidationError("Database connection error. Please check your configuration.");
            return;
        }
        ...
        try {
            ...
            console.error(error);
            setValidationError(`Failed to submit registration: ${error.message}. Please try again or contact support.`);
        } finally {
            setIsLoading(false);
        }
    };
            ...
            <div className="max-w-4xl mx-auto px-6">
                ...
                <AnimatePresence mode="wait">
                    <motion.div key={currentStep} initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.3 }} className="bg-white/60 backdrop-blur-md p-8 rounded-2xl shadow-lg border border-yellow-200/50">
                                ...
                                <div className="space-y-8">
                                    {Array.from({ length: formData.teamSize }).map((_, index) => (
                                        <div key={index} className="p-6 bg-gray-50 rounded-lg border border-gray-200">
                                            ...
                                            <div className="grid md:grid-cols-2 gap-4">
                                                <input type="text" value={formData.members[index].fullName} onChange={(e) => handleMemberChange(index, 'fullName', e.target.value)} placeholder="Full Name" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500" />
                                                <select value={formData.members[index].grade} onChange={(e) => handleMemberChange(index, 'grade', e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 disabled:bg-gray-200/70" disabled={index > 0}>
                                                    <option value="">Select Grade</option>
                                                    <option value="7th">7th</option>
                                                    <option value="8th">8th</option>
                                                    <option value="9th">9th</option>
                                                </select>
                                                <input type="email" value={formData.members[index].email} onChange={(e) => handleMemberChange(index, 'email', e.target.value)} placeholder="Email Address" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500" />
                                                <input type="tel" value={formData.members[index].phoneNumber} onChange={(e) => handleMemberChange(index, 'phoneNumber', e.target.value)} placeholder="Phone Number" className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500" />
                                            </div>
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="text-center space-y-6">
                                    ...
                                    <div className="flex flex-col sm:flex-row items-center justify-center gap-8">
                                        <div className="text-center">
                                            ...
                                            <img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=https://www.instagram.com/reelhaus.hyd/" alt="Instagram QR Code" className="rounded-lg shadow-md" />
                                        </div>
                                        <div className="text-center">
                                    ...
                                    <div className="flex items-start justify-center p-4 bg-blue-50 rounded-lg border border-blue-200 max-w-md mx-auto">
                                        <input type="checkbox" id="follow" checked={hasFollowedInstagram} onChange={(e) => setHasFollowedInstagram(e.target.checked)} className="mt-1 mr-3 h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
                        ...
                        {currentStep === 5 && (
                            <motion.div key="manual-payment-step" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.3 }}>
                                <h2 className="text-2xl font-bold text-gray-800 mb-6 flex items-center"><Upload className="w-6 h-6 mr-3 text-yellow-500" /> Manual Payment & Confirmation</h2>
                                <div className="space-y-6">
                                    ...
                                    <div className="flex flex-col items-center p-6 bg-gray-50 rounded-lg border border-gray-200">
                                        ...
                                        <img src={qrCodeSrc} alt="Payment QR Code" className="rounded-lg shadow-md mb-4 w-48 h-48" />
                                    ...
                                    <div className="grid md:grid-cols-2 gap-6">
                                        <div>
                                            ...
                                            <input type="text" value={transactionId} onChange={(e) => setTransactionId(e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500" placeholder="Enter transaction ID" />
                                        </div>
                                        <div>
                                            ...
                                            <div className="flex items-center">
                                                <label className="flex-1 cursor-pointer bg-yellow-100 text-yellow-800 p-3 rounded-lg border border-yellow-300 hover:bg-yellow-200 transition-colors flex items-center justify-center">
                                                    <input type="file" accept="image/*" onChange={(e) => setPaymentScreenshot(e.target.files ? e.target.files[0] : null)} className="sr-only" />
                ...
                <div className="flex justify-between mt-8">
                    ...
                    {currentStep < 5 ? (
                        ...
                        <button onClick={handleFinalSubmission} disabled={isLoading || !isStepValid()} className="flex items-center justify-center px-8 py-3 bg-green-500 text-white rounded-lg font-semibold hover:bg-green-600 disabled:opacity-50 w-60">
                            {isLoading ? (<div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div>) : "Complete Registration"}
                        </button>
                    )}
                </div>
