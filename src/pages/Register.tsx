...
const Register: React.FC = () => {
  ...
  useEffect(() => {
    ...
    return () => {
        if (document.body.contains(cashfreeScript)) document.body.removeChild(cashfreeScript);
  ...
  const interests = [
    'Gaming & Esports', 'Technology & Coding', 'Art, Design & Video', 'Sports & Strategy', 'Public Speaking & Debating', 'Business & Marketing'
  ];
  ...
  const handlePayment = async () => {
    ...
    try {
      const response = await fetch('http://localhost:3001/api/create-payment-session', {
        ...
        body: JSON.stringify({
          amount: parseFloat(paymentDetails.total.toFixed(2)),
          customer_details: {
      ...
      // Corrected Logic: Use .then() to handle the async result of checkout
      cashfree.checkout({
        ...
        if (error instanceof TypeError && error.message.includes('Failed to fetch')) {
           setValidationError("Connection Error: Could not connect to the backend server. Please ensure it's running and you're on the same network.");
        } else {
           setValidationError(error.message || "An unknown error occurred. Please try again later.");
        }
        setIsLoading(false);
    }
  };
      ...
      <div className="max-w-4xl mx-auto px-6">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
          ...
          <div className="bg-white/60 backdrop-blur-md p-6 rounded-2xl shadow-lg border border-yellow-200/50 mt-8 max-w-2xl mx-auto">
            <p className="text-gray-700 leading-relaxed">Hey Innovators! Get ready for an epic experience where your ideas can shine. InnovateX25 is your chance to team up with friends, tackle fun challenges, and show everyone what you've got!</p>
          </div>
        </motion.div>
                    ...
                    <div key={index} className="p-6 bg-gray-50 rounded-lg border border-gray-200">
                      ...
                      <div className="grid md:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <input type="email" value={formData.members[index].email} onChange={(e) => handleMemberChange(index, 'email', e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-transparent" placeholder="Enter email address"/>
                        </div>
                        <div>
            ...
            {currentStep === 5 && (() => {
              ...
              return (
                <div>
                  ...
                  <div className="space-y-6">
                    {postPaymentError && (<div className="p-4 bg-red-100 border-l-4 border-red-500 text-red-700"><div className='flex'><AlertTriangle className='h-5 w-5 text-red-500 mr-3'/><div><p className="font-bold">Registration Error</p><p>{postPaymentError}</p></div></div></div>)}
